				...
				echo '<td colspan="2" align="center" style="background: green;color: white;">Status Transaksi :';
					...
					if ($tglsekarang > $batasbayar) {
						...
						if ($status == "Pending...") {
							...
							$sqlw = $pdo->query("SELECT * FROM pembayaran WHERE idpesan='$idpesan'");
							/*if($sqlw === false) {
								die("Error in sql query: " . $pdo->errorInfo()[2]);
							};*/
							$dataw = $sqlw->fetch();
							/*if($dataw === false) {
								die("no data found for idpesan='$idpesan'");
						    };*/
							$idbayar = $dataw['idpesan'] ?? null;
							if ($idbayar == $idpesan) {
